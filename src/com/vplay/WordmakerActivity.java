...
public class WordmakerActivity extends VPlayActivity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		setContentView(R.layout.activity_wordmaker);
		
		//Контекст
		Context context = this;
		
		//ИД xml файла внешнего вида строки таблицы
		int item_layout_id = R.layout.activity_wordmaker_tableitem;
		
		//Информация о ячейках строки таблицы
		List<ColumnInfo> column_info = new ArrayList<ColumnInfo>();
		column_info.add(new ColumnInfo(EColumnType.IMAGE_COLUMN, R.id.wordmaker_device_image));
		column_info.add(new ColumnInfo(EColumnType.TEXT_COLUMN, R.id.wordmaker_correct_words_text));
		column_info.add(new ColumnInfo(EColumnType.TEXT_COLUMN, R.id.wordmaker_incorrect_words_text));
		column_info.add(new ColumnInfo(EColumnType.TEXT_COLUMN, R.id.wordmaker_points_text));
		
		//ИД LinearLayout в который будет вставлена таблица
		LinearLayout main_linear_layout = (LinearLayout) findViewById(R.id.wordmaker_table_layout);
		...
		words_list = get_game_content_list(game_theme_string);
	...
	public void on_click_wordmaker_end_game_button(View v)
	{
		this.finish();
	}
	...
	public void update_key(short build_number, byte device_number, EDeviceKey key)
	{
		...
		else if(key.equals(EDeviceKey.TWO_BUTTON))
		{
			...
			//Отправить информаци
			usb_service.send_display_string_command(build_number, device_number, player.get_current_word().toUpperCase());
		}
		else if(key.equals(EDeviceKey.DELETE_BUTTON))
		{
			...
			//Удалить последнюю букву слова
			player.delete_last_letter();
	...
	public void update_rfid(short build_number, byte device_number, String rfid)
	{
		...
		player.add_new_letter(letter);
		
		//Выслать обновленную строку
		usb_service.send_display_string_command(build_number, device_number, player.current_word.toUpperCase());
	...
	private List<String> get_game_content_list(String filename)
	{
		...
		try
		{
			is = asset_manager.open("wordmaker_game/" + filename);
		}
		catch(IOException e)
		{
	...
	public class Player
	{
		...
		public void delete_last_letter()
		{
		...
		public void add_new_letter(String letter)
		{
			current_word += letter;
		}
